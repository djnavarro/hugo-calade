...
When you do this, hugodown will download the slum template and build the example site. However, consistent with the hugodown principle of separating R from Hugo, what it will do is take the `.Rmd` files that comprise my site, and build them to `.md` files. If you want to use Hugo to build those `.md` files into the final webpage, use the following command:

```{r, eval=FALSE}
hugodown::hugo_build("path_to_blog_folder")
```

At this point you should have the website in the `docs` folder. 
