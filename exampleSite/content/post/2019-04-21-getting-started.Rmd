...
## Create a slum blog using blogdown

```{r, eval=FALSE}
blogdown::new_site(
  dir = "path/to/desired/blog", 
...
## Create a slum blog using hugodown

Something like this, presumably? 

```{r, eval=FALSE}
hugodown::create_site_slum()
```
...
Note that if you are using blogdown rather than hugodown, the theme folder will have been automatically renamed from `"slum"` to `"hugo-slum"`, and this will be reflected in the `config.toml` file.

In any case, take a look at the config file, change a couple of things if you like, and then you're ready to start blogging!
